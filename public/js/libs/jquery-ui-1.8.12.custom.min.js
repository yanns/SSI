...
function Datepicker() {
	...
	this._defaults = { // Global defaults for all the date picker instances
		...
		showDays: false, // False to hide the days //added by ADESSO
		showWeek: false, // True to show week of the year, false to not show it
		showMonths: false, // False to hide the days //added by ADESSO
...
$.extend(Datepicker.prototype, {
	...
	/* Hide and update the date picker from view. 
	 * added by ADESSO */
	_hideAndUpdateDatepicker: function(id) {
		var inst = this._curInst;
		var showDays = this._get(inst, 'showDays');
		if (!showDays) {
			inst.selectedDay = 1;
		}
	...
	parseDate: function (format, value, settings) {
		...
		// change Adesso - if the month or days are not shown, init them
		var showMonths = (settings ? settings.showMonths : null) || this._defaults.showMonths;
		var showDays = (settings ? settings.showDays : null) || this._defaults.showDays;
		if (!showDays && day == -1) {
			day = 1;
		}
		if (!showMonths && month == -1) {
			month = 1;
		}
		// end of change adesso
	...
	_generateHTML: function(inst) {
		...
		var maxDate = this._getMinMaxDate(inst, 'max');
		var showDays = this._get(inst, 'showDays');
		var showMonths = this._get(inst, 'showMonths');
		...
		// change ADESSO: do not show button for prev month is showMonths = false
		var prev = (showMonths && this._canAdjustMonth(inst, -1, drawYear, drawMonth) ?
		...
		nextText = (!navigationAsDateFormat ? nextText : this.formatDate(nextText,
			this._daylightSavingAdjust(new Date(drawYear, drawMonth + stepMonths, 1)),
			this._getFormatConfig(inst)));
		// change ADESSO: do not show button for next month is showMonths = false
		var next = (showMonths && this._canAdjustMonth(inst, +1, drawYear, drawMonth) ?
		...
		// changed by ADESSO, close calls _hideAndUpdateDatepicker instead of _hideDatepicker
		var controls = (!inst.inline ? '<button type="button" class="ui-datepicker-close ui-state-default ui-priority-primary ui-corner-all" onclick="DP_jQuery_' + dpuuid +
			'.datepicker._hideAndUpdateDatepicker(\'#' + inst.id + '\');">' + this._get(inst, 'closeText') + '</button>' : '');
		// changed by ADESSO - when not showDays, hide the button 'TODAY' 
		var buttonPanel = (showButtonPanel) ? '<div class="ui-datepicker-buttonpane ui-widget-content">' + (isRTL ? controls : '') +
			(showDays && this._isInRange(inst, gotoDate) ? '<button type="button" class="ui-datepicker-current ui-state-default ui-priority-secondary ui-corner-all" onclick="DP_jQuery_' + dpuuid +
		...
		for (var row = 0; row < numMonths[0]; row++) {
			...
			for (var col = 0; col < numMonths[1]; col++) {
				...
				// change ADESSO: when showDas -> display:none for the table for the days
				calender += '<div class="ui-datepicker-header ui-widget-header ui-helper-clearfix' + cornerClass + '">' +
					(/all|left/.test(cornerClass) && row == 0 ? (isRTL ? next : prev) : '') +
					(/all|right/.test(cornerClass) && row == 0 ? (isRTL ? prev : next) : '') +
					this._generateMonthYearHeader(inst, drawMonth, drawYear, minDate, maxDate,
					row > 0 || col > 0, monthNames, monthNamesShort) + // draw month headers
					'</div><table class="ui-datepicker-calendar"' + (showDays ? '' : ' style="display:none"') + '><thead>' +
					'<tr>';
	...
	_generateMonthYearHeader: function(inst, drawMonth, drawYear, minDate, maxDate,
		...
		if (secondary || !changeMonth)
			// change Adesso: do show show Months
			monthHtml += (showMonths? '<span class="ui-datepicker-month">' + monthNames[drawMonth] + '</span>' : '');
			// end of change adesso
		else {
	...
	_getFormatConfig: function(inst) {
		...
		return {shortYearCutoff: shortYearCutoff,
			...
			showMonths: this._get(inst, 'showMonths'), showDays: this._get(inst, 'showDays')}; // added by Adesso
	},
